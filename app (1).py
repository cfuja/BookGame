...
class BookGenerator:
    ...
    def add_character(self, name, age, gender, occupation, personality, backstory, physical_traits, motivations, relationships):
        ...
        gender = gender.strip() or "Not specified"
        occupation = occupation.strip() or "Not specified"
        personality = personality.strip() or "Not specified"
        backstory = backstory.strip() or "Not specified"
        physical_traits = physical_traits.strip() or "Not specified"
        motivations = motivations.strip() or "Not specified"
...
def index():
    session["generator"] = session.get("generator", None)
...
def add_character():
    if request.method == "POST":
        try:
            ...
            if not session.get("generator"):
                session["generator"] = BookGenerator().__dict__
            
            generator = BookGenerator()
            generator.__dict__.update(session["generator"])
            ...
            session["generator"] = generator.__dict__
            ...
            flash(f"Error: Invalid input. {e}", "error")
...
def generate_plot():
    try:
        if not session.get("generator"):
            flash("Error: No book data available.", "error")
            return redirect(url_for("index"))
...
@app.route("/preview_structure", methods=["POST"])
...
def generate_book():
    try:
        ...
        session["book_content"] = book_content
        session["generator"] = generator.__dict__
...
def book_display():
    book_content = session.get("book_content", "No book generated yet.")
    return render_template("book_display.html", book_content=book_content)

@app.route("/download_book")
def download_book():
    book_content = session.get("book_content", "")
    if not book_content:
        flash("Error: No book available to download.", "error")
        return redirect(url_for("index"))
    
    generator = BookGenerator()
    generator.__dict__.update(session.get("generator", {}))
    buffer = BytesIO(book_content.encode('utf-8'))
    filename = f"{secure_filename(generator.title)}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
