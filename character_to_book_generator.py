...
class Character:
    def __init__(self, name, age, gender, occupation, personality, backstory, physical_traits, motivations, relationships=None):
        ...
        self.development = []  # Tracks character growth through chapters
...
"""
    ...
    def generate_chapter(self, chapter_num, pages_per_chapter, arc_stage):
        ...
        # Incorporate a key plot point if applicable
        plot_point = ""
        if self.key_plot_points and random.random() < 0.2:  # 20% chance to include a plot point
...
"""
        ...
        # Export to file
        filename = f"{self.title.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        try:
